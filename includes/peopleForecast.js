...
function populateMonthArray(){ // Populates the cells with the month name (MMM-YY)
	var x = document.getElementById('months'); // gets the selector with the id "months" so that we can find out what value it's set at
	y = Number(x.value); // set y to be the selected value, being the offset required for the month selected e.g. -2 would be two months in the past
	
	let monthArray = generateMonthRange(y); // creates an array with the months required e.g. ['Oct-24', 'Nov-24', 'Dec-24', 'Jan-25', 'Feb-25', 'Mar-25', 'Apr-25', 'May-25', 'Jun-25', 'Jul-25', 'Aug-25', 'Sep-25', 'Oct-25', 'Nov-25']
	
	for (let i = 1; i < monthArray.length; i++) { // go through each of the months in the monthArray and pop them into the Table Header slots for them
		document.getElementById('month'+i+'TD').innerHTML = monthArray[i-1]; // monthArray reference needs to be -1 as the array starts at 0
	}
}

function populateMonthArray(selectID){ // Populates the cells with the month name (MMM-YY)
	sID = selectID;
	
	if(sID == null){
		sID = 'months';
	}
	var x = document.getElementById(sID); // gets the selector with the id "months" so that we can find out what value it's set at
	y = Number(x.value); // set y to be the selected value, being the offset required for the month selected e.g. -2 would be two months in the past
	
	let monthArray = generateMonthRange(y); // creates an array with the months required e.g. ['Oct-24', 'Nov-24', 'Dec-24', 'Jan-25', 'Feb-25', 'Mar-25', 'Apr-25', 'May-25', 'Jun-25', 'Jul-25', 'Aug-25', 'Sep-25', 'Oct-25', 'Nov-25']
	
	for (let i = 1; i < monthArray.length; i++) { // go through each of the months in the monthArray and pop them into the Table Header slots for them
		currentCell = document.getElementById('month' + i + 'TD'); // set the variable to be used below
		if(currentCell != null){ // if the element exists, execute this code else it's not needed
			document.getElementById('month'+i+'TD').innerHTML = monthArray[i-1]; // monthArray reference needs to be -1 as the array starts at 0
		}
	}
}
...
function populateTableHead() { // TO BE DELETED
	let a = getCookie("aMonths");
	let o = getCookie("oMonths");
	
	// Example month array generator (you must define generateMonthRange separately)
	const monthArray = generateMonthRange(); // e.g. ["Jan 2025", "Feb 2025", ...]

	// Get the <thead> element and clear it
	const thead = document.querySelector('thead');
	thead.innerHTML = ''; // Clear any existing content

	// Create a new <tr> to contain the <th> cells
	const row = document.createElement('tr');

	// Static columns
	const tx = ["", "REF", "Name", "Salary", "Actual", "Forecast", "Variance",""];

	for (let i = 0; i < tx.length; i++) {
		const th = document.createElement('th');
		th.textContent = tx[i];
		th.className = tx[i].toLowerCase().replace(/\s+/g, '-'); // Safe class name
		row.appendChild(th);
	}

	// Dynamic month columns
	let z = 1
	for (let i = 0; i < tMonths; i++) { // Includes the minus 1 as the monthArray length is too long!!!
		const th = document.createElement('th');
		th.textContent = monthArray[i];
		th.id = "month" + z + "TD";
		if (i == a - 1){
			th.className = "cMonth"; // Mark the column as the current month for CSS purposes	
		} else if (i < a) {
			th.className = "pMonth"; // Mark the column as a prior month for CSS purposes
		} else {
			th.className = "fMonth"; // Mark the column as a future month for CSS purposes
		}

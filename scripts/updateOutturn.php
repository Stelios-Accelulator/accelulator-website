...
if ($numRows === 0) {
	error_log("ℹ️ No matching rows found — inserting new row");

	$insertStmt = $pdo->prepare("
		INSERT INTO {$table_outturn} (RES_ROL, DATE, EMP_KEY, TYPE, VALUE, CREATED)
		VALUES (:res_rol, :date, :emp_key, :typeRef, :value, NOW())
	");

	$insertSuccess = $insertStmt->execute([
		':res_rol' => $res_rol,
		':date' => $endDate->format('Y-m-d'), // Last day of the month
		':emp_key' => $resourceRef,
		':typeRef' => $typeRef,
		':value' => $value
	]);
	...
	if (!$insertSuccess) {
		...
		echo json_encode([
			...
			"pdo_error" => $insertStmt->errorInfo()
		]);
		exit;
	}
	...
	echo json_encode([
		"status" => "inserted",
		"message" => "No existing rows — inserted new entry",
		"res_rol" => $res_rol,
		"type" => $type,
		"typeRef" => $typeRef,

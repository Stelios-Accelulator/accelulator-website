...
<script>
			...
			if (data.status === 'success') {
				...
				try{
				  ...
				  populateForecastOptions();
					selectLatestPublishedForecastOnLoad();
				  createTable();
	...
	} // Runs when the user hits Attach: Calls attachRole.php and then refreshes the views
	...
	function createDepartmentMenu(){ // Creates and shows the Department menu
		...
		<div id='draggableEmployeeMenuHeader' class='menuHeader'><strong>Department Menu</strong> <button onClick='destroyMenu("menuContainer");'><strong><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-6 toolbarIcon" height="1.5em" width="1em"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" /></svg></strong></button></div>
		<div class='menuRow'><input id='newDepartmentName' type='text' placeholder='New Department'></div>
		<div class='buttonGroup'><button onclick='actionAddDepartment();'>Add</button></div>
		`;
	...
	function createPaytypeMenu(){
		...
		paytypeMenu.innerHTML = `<div class='menuHeader'><strong>Allocate Pay Type</strong><button onClick='destroyMenu("menuContainer");'>
	...
	function createSettingsMenu(){
		...
		settingsMenu.innerHTML = `
			<div class='menuHeader'><strong>Settings</strong><button onClick='destroyMenu("menuContainer");'>
			<svg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 24 24' stroke-width='1.5' stroke='currentColor' class='size-6 toolbarIcon' height='1.5em' width='1.5em'>
			  <path stroke-linecap='round' stroke-linejoin='round' d='M6 18 18 6M6 6l12 12' />
			</svg>
			</button></div>
